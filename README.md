...
* [3. 简写字符集](#3-简写字符集)
* [4. 环视](#4-环视)
	* [4.1 ?=... 肯定顺序环视](#41--肯定顺序环视)
	* [4.2 ?!... 否定顺序环视](#42--否定顺序环视)
	* [4.3 ?&lt;= ... 肯定逆序环视](#43---肯定逆序环视)
	* [4.4 ?&lt;!... 否定逆序环视](#44--否定逆序环视)
* [5. 标志](#5-标志)
...
顺序环视和逆序环视都属于**非捕获簇**(用于匹配不在匹配列表中的格式).
顺序环视用于判断所匹配的格式是否在另一个确定的格式之后.

例如, 我们想要获得所有跟在 `$` 符号后的数字, 我们可以使用肯定逆序环视`(?<=\$)[0-9\.]*`.
...
`?=...` 肯定顺序环视, 表示从`?=...`定义的规则开始位置匹配表达式.

返回结果只满足第一部分表达式.
定义一个顺序环视(存在)要使用 `()`. 在括号内部使用一个问号和等号: `(?=...)`. 

顺序环视的内容写在括号中的等号后面.
例如, 表达式 `[T|t]he(?=\sfat)` 匹配 `The` 和 `the`, 在括号中我们又定义了肯定顺序环视 `(?=\sfat)` ,即 `The` 和 `the` 后面紧跟着 `(空格)fat`.
...
否定顺序环视 `?!` 用于筛选所有匹配结果, 即不从`?!...`定义的规则开始位置匹配表达式.
`否定顺序环视`  定义和 `肯定顺序环视` 一样, 区别就是 `=` 替换成 `!` 也就是 `(?!...)`. 
...
肯定逆序环视 记作`(?<=...)` 用于筛选所有匹配结果, 表示从`?<= ...`定义的规则末端位置匹配表达式.
...
否定逆序环视 记作 `(?<!...)` 用于筛选所有匹配结果, 即不从`?<= ...`定义的规则末端位置匹配表达式.
...
## 贪婪(匹配优先)与非贪婪(忽略优先)简说
- 标准匹配量词(?,*,+,{min,max})都是匹配优先
- 简单来说,匹配优先会匹配最长的结果,忽略优先会匹配最短的结果

[在线练习](https://regex101.com/r/HrIFHr/3)

## 案例

### 校验用户名
 校验用户名,允许字母,数字,下划线,长度三位以上,必须同时包含大小写和数字.

[在线练习](https://regex101.com/r/9pnI5t/1)
